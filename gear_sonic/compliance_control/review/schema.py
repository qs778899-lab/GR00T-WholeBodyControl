...
class EvaluationTrace:
    ...
    def __post_init__(self) -> None:
        ...
        action_array = np.asarray(self.policy_actions)
        if action_array.ndim != 2 or action_array.shape[0] != row_count:
            raise ValueError(
                "policy_actions must have shape [row, caller_owned_action]"
            )
        if action_array.shape[1] <= 0:
            raise ValueError("policy_actions must contain at least one action")
        field_specs = {
            ...
            "policy_actions": ((row_count, action_array.shape[1]), "float"),
        ...
        if np.any(self.residual_enabled & ~self.compliance_enabled):
